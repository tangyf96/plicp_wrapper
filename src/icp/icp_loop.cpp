...
// 保存结果的路径
std::string pre = "/home/yifan/calibration/plicp/result/";
...
int icp_loop(struct sm_params*params, const double*q0, double*x_new, 
	...
	LDP laser_ref = params->laser_ref;  /// 修改
    ...
    if (params->icp_iteration_visible)
    {
		{
			std::stringstream ss;
			ss << pre << "ref-" << params->timeStamp << ".txt";
			FILE *out = fopen(ss.str().c_str(), "w");
			try {
				for (int i=0; i<laser_ref->nrays; ++i) {
					fprintf(out, "%f %f %d\n",laser_ref->points[i].p[0], laser_ref->points[i].p[1] ,laser_ref->valid[i] );
				}
				fclose(out);
			} catch(...) {
		...
		{
			std::stringstream ss;
			ss << pre << "sen-" << params->timeStamp << ".txt";
			FILE *out = fopen(ss.str().c_str(), "w");
			try {
				for (int i=0; i<laser_sens->nrays; ++i) {
					fprintf(out, "%f %f %d\n",laser_sens->points[i].p[0],laser_sens->points[i].p[1] ,laser_sens->valid[i] );
				}
				fclose(out);
			} catch(...) {
				printf("[icp][error] write to icp iteration refference file failed!\n");
